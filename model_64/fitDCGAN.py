...
start_img = PIL.Image.open(  "foo_2.png").convert("RGB")
start_img = start_img.resize((64, 64), PIL.Image.ANTIALIAS)
start_img_np = np.array(start_img)/255.0
fz = tf.Variable(start_img_np, tf.float32)
fz = tf.expand_dims(fz, 0)
fz = tf.cast(fz,tf.float32)


zp = tf.Variable(np.random.normal(size=(1,200)), dtype=tf.float32)
#fzp = generator(zp,training=False)
#loss = tf.reduce_sum(  tf.square(fz - fzp) )

#tf.keras.losses.mean_squared_error( y_true= fz , y_pred= fzp)
...
def step():
        with tf.GradientTape() as tape:
            ...
            loss =   tf.reduce_sum ( tf.square( fz - generator(zp,training=False)))
        gradients = tape.gradient(loss, [zp] )
        optimizer.apply_gradients(zip(gradients, [zp]))
        return loss
last_loss =1e99
for i in range(10000):
   tr_loss_v= step()
   if ( abs(tr_loss_v - last_loss) < 1e-4) :break
   if (i%5 == 0 ) : print(i, tr_loss_v)
   last_loss = tr_loss_v + 0





imgs =  generator(zp).numpy()
imgs = ((imgs + 0.5) * 128).astype(np.uint8)
PIL.Image.fromarray(imgs[0]). resize((128, 128), PIL.Image.ANTIALIAS).save(  "foo_2_rec.png")
