...
def make_generator_model(original_w, noise_dim):
    ...
    model.add(
        tf.keras.layers.Conv2DTranspose(filters=8 * g, kernel_size=k, strides=(2, 2), padding="same", use_bias=False,
    ...
    model.add(
        tf.keras.layers.Conv2DTranspose(filters=4 * g, kernel_size=k, strides=(2, 2), padding="same", use_bias=False,
                                        kernel_initializer='he_uniform'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU(0.2))
    print(model.output_shape)
    ...
    model.add(
        tf.keras.layers.Conv2DTranspose(filters=2 * g, kernel_size=k, strides=(2, 2), padding="same", use_bias=False,
                                        kernel_initializer='he_uniform'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU(0.2))
    print(model.output_shape)

    model.add(tf.keras.layers.Conv2DTranspose(filters=g, kernel_size=k, strides=(2, 2), padding="same", use_bias=False,
                                              kernel_initializer='he_uniform'))
    model.add(tf.keras.layers.BatchNormalization())
    model.add(tf.keras.layers.LeakyReLU(0.2))
    print(model.output_shape)
...
def make_discriminator_model(original_w, noise_var):
    ...
    model.add(tf.keras.layers.Conv2D(g // 4, k, strides=(2, 2), padding='same', use_bias=False,
                                     input_shape=[None, original_w, original_w, 3]))
    model.add(tf.keras.layers.BatchNormalization())
